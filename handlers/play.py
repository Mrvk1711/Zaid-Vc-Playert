...
PLAYMSG_BUTTONS = InlineKeyboardMarkup(
    [
            [
                InlineKeyboardButton("ꜱᴜᴘᴘᴏʀᴛ 🦋", url=f"https://t.me/terayaarhoomai"),
                InlineKeyboardButton(" ᴄʜᴀɴɴᴇʟ 🦋", url=f"https://t.me/maxopeditz"),
            ],
...
async def play(_, message: Message):
    ...
    if message.chat.id in callsmusic.active_chats:
        ...
        await response.delete()
        await message.reply_photo(thumb, caption="**Playing Your Song 🎧...** \n**Lucky user: {}**".format(message.from_user.mention()), reply_markup=PLAYMSG_BUTTONS)
...
async def nplay(_, message: Message):
    ...
    bttn = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("🦋ᴀꜱꜱɪꜱᴛᴀɴᴛ🦋", url=f"https://t.me/cute_boy724")
            ],[
    ...
    if message.chat.id in callsmusic.active_chats:
        ...
        MENTMEH = message.from_user.mention()
        await lel.delete()
        await message.reply_photo(thumb, caption=f"**Your Song Queued at position** `{position}`! \n**🦋ʟᴜᴄᴋʏ ᴜꜱᴇʀ: {MENTMEH}**", reply_markup=PLAYMSG_BUTTONS)
    else:
        thumb = THUMB_URL
        await callsmusic.set_stream(message.chat.id, file)
        await lel.delete()
        await message.reply_photo(thumb, caption="**🦋ᴘʟᴀʏɪɴɢ...** \n**🦋ʟᴜᴄᴋʏ ᴜꜱᴇʀ: {}**".format(message.from_user.mention()), reply_markup=PLAYMSG_BUTTONS)
